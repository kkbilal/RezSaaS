...
- Kalıcı hesap kapatma sanction endpoint'inden uygulanmaz; iki farklı step-up admin, itiraz penceresi ve Identity hesap kapatma orchestration'ı kullanılır.
...
## Uygulanan İtiraz ve Kalıcı Kapatma Akışı

- Müşteri authenticated ve rate limited yüzeyde yalnızca kendi strike, aktif bloklayıcı sanction veya closure case kaydına itiraz edebilir.
- Kabul edilen itiraz strike/sanction'ı revoke eder veya closure case'i `CancelledByAppeal` durumuna taşır.
- Customer response'unda internal neden, review reason veya admin actor kimliği gösterilmez.
- Kalıcı kapatma yalnızca `High` risk eşiği, iki farklı `PlatformAdminWithStepUp`, varsayılan 7 günlük itiraz penceresi ve açık itiraz bulunmamasıyla yürütülür.
- Aktif strike sayısı execution anında yeniden hesaplanır; risk seviyesi artık `High` değilse kapatma ilerlemez.
- Platform rolü veya aktif tenant membership taşıyan hesap kapatılamaz.
- Kapatma tamamlandığında Identity hesabı terminal `Closed` olur, eski authenticated istekler reddedilir ve Admin geçmişinde `PermanentClosure` kaydı üretilir.
